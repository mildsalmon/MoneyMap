...
- 기본 입력은 왼쪽 차변·오른쪽 대변의 고정된 계정 트리다. 유형 탭과 계정 select를 사용하지 않는다. 실제 사용 가능한 말단 계정만 라디오 항목으로 선택한다.
- 선택 요약에 이름·현재 경로·자동/직접 선택 상태를 표시한다. 검색과 그룹 접기는 선택을 바꾸지 않는다.
- 아이템 아래에 선택 메모를 둔다. 마지막 조합은 계정만 불러오며 날짜·금액·메모는 불러오지 않는다. 저장 후 같은 초안이면 금액과 메모를 비우고 금액으로 포커스를 돌린다.
- 분할 입력은 보조 모드이며 한 행의 선택기만 펼친다. 복귀로 내용을 잃을 수 있으면 현재 초안을 유지하고 이유를 표시한다.
- 720px 이하에서 차변 다음 대변을 쌓는다. 계정 영역 위에 선택 요약, 화면 아래에 금액·저장 바를 둔다. 보이는 화면 높이 480px 이하에서는 둘 다 일반 흐름으로 내려 키보드 공간을 확보한다.
- [승인 설계](docs/designs/transaction-input.md), [구현 검증](docs/verification/transaction-input.md).
