...
**DONE_WITH_CONCERNS.** 기능·표시 문제 3건과 테스트 공백 2건을 보완했다. 확인한 미해결 코드 결함은 없다. 사용량 오류가 났던 Red Team 검토는 재시도에서 완료했고, 긴 아이템의 모바일 실행취소 문제를 추가로 찾아 수정했다. 실기기 모바일 검증과 최신 main 통합은 남아 있다.
...
- 이번 수정은 아래 파일에 **미커밋 상태**로 남겼다. 커밋·푸시·PR 작성은 하지 않았다.
...
- main의 VERSION은 이미 **0.4.0.0**, 이 브랜치는 **0.3.0.0**이다. 큐의 앞선 PR은 0개이고 조회한 다음 patch 슬롯은 **0.4.1.0**이다. 릴리스 시 최신 main 통합과 버전 조정을 해야 한다. 이번 검토가 최신 main과 합친 결과를 테스트한 것은 아니다.
