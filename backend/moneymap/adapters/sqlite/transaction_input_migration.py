...
from moneymap.domain.transaction_input import normalize_item_key_v4
...
def migrate_transaction_input(conn):
    ...
    conn.create_function("moneymap_item_key", 1, normalize_item_key_v4, deterministic=True)
    try:
        ...
        conn.execute(
            ...
            f"WHEN id IN (SELECT transaction_id FROM ({V4_OPENING_BALANCES_SQL})) THEN 'system' "
